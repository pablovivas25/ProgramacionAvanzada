	...
	public void resolver() {
		...
		for(int i=1;i<this.imagen_descomprimida.length();i++) {
			if((""+this.imagen_descomprimida.charAt(i)).equals(cad)) {
				...
				if(cont>4) {
					this.imagen_comprimida_out+="(";
					this.imagen_comprimida_out+=cad;
					this.imagen_comprimida_out+=""+cont;
					this.imagen_comprimida_out+=")";
				}else {
					if(cont!=0)
						this.imagen_comprimida_out+=cad.repeat(cont);
					else
		...
		System.out.println(this.imagen_descomprimida_out);
	}
